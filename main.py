...
def run_chains(
    ...
    wave_samplers = [
        wave_sampling.Wave_Sampling(
            initial_samples[i],
            current_sample_obs[i],
            current_sample_lls[i],
            sample_means[i],
            grid_sizes,
            adaptive_matrices[i],
            epsilons[i],)
    ...
    if nuts:
        ...
        funcs = [
            functools.partial(
                wave_samplers[i].sample_multi_fidelity_nuts,
        ...
        funcs = [
            functools.partial(
                wave_samplers[i].sample_multi_fidelity_rw,
                save_filename=chain_filenames[i],
                n_samples=n_samples,
                matrix_adapt_start=matrix_adapt_start,)
            for i in range(n_chains)
    ...
    wave_sampling.detransform_samples(
        save_filename, save_filename[:-4] + "_detransformed.npy"
        )
...
if __name__ == "__main__":
    ...
    if not isinstance(grid_sizes, list):
        raise ValueError(
            "grid_sizes must be inputted as a list (even if it only contains one element)."
        )

    if epsilon_adapt > n_samples:
        raise ValueError(
            "The burn-in phase must finish within the initial sampling session (epsilon_adapt <= n_samples)"
            )

    if epsilon_adapt > n_SIR_samples and SIR_stages != 0:
        raise ValueError(
            "The burn-in phase must finish within each sequential resampling session (epsilon_adapt <= n_SIR_samples)"
            )

    if n_chains == 1 and SIR_stages != 0:
        raise ValueError(
            "One chain cannot be sequentially resampled; please change the number of chains or set sequential_resample = 0."
            )

    # Check if save_filename exists, if not, start a new chain, 
    # else continue the chains
    if exists(save_filename):
        ...
        (
            ...
            epsilons,
            _,
        ) = save_functions.load_data(save_filename)

        # Check that the grid sizes match
        if not np.array_equal(grid_sizes, old_grid_sizes):
            raise ValueError(
                "The specified grid_sizes do not match the previously-used grid_sizes."
                )
        ...
        if len(all_model_samples.shape) <= 3:  # There is only one chain
            ...
            initial_samples = np.array(
                [chain[-1][-1] for chain in all_model_samples]
                )

        # Do NOT include the final sample since that will be the next sample 
        ...
        if input_sample == "random":
            ...
            gauss_params = multivariate_normal.rvs(
                mean=wave_specs.PRIOR_MEAN,
                cov=np.diag(wave_specs.PRIOR_COV_DIAG),
                size=n_chains,
                )
            beta_param = beta.rvs(
                a=wave_specs.PRIOR_A, b=wave_specs.PRIOR_B, size=n_chains
                )
    ...
    if initial_samples.shape[0] != n_chains:
        raise ValueError(
            "The specified number of chains does not match the number of initial states."
            )
    ...
    for resample in range(SIR_stages):
        ...
        chain_filenames = [
            save_filename[:-4]
            + f"_{SIR_stages}resamples_{n_SIR_samples}points_chain{i + 1}.npy"
            for i in range(n_chains)
            ]

        # Save all resample data to the same file
        run_chains(
            save_filename[:-4] + f"_{SIR_stages}resamples_{n_SIR_samples}points.npy",
            chain_filenames,
            initial_samples,
            current_sample_obs,
            current_sample_lls,
        ...
        (
            ...
            _,
            _,
        ) = save_functions.load_data(
            save_filename[:-4] + f"_{SIR_stages}resamples_{n_SIR_samples}points.npy"
        )

        # Extract the initial_samples and log probs from the last sample 
        # accepted by the highest fidelity model
        initial_samples = np.array([chain[-1][-1] for chain in all_samples])
        high_f_sample_log_probs = np.array(
            [chain[-1][-1] for chain in all_model_log_probs]
            )

        # Create (normed) weights using the log prob of each chain's sample
        weights = np.exp(high_f_sample_log_probs) / np.sum(
            np.exp(high_f_sample_log_probs)
            )

        # Draw n_chains indices representing which samples to start the new 
        # chains with
        new_indices = np.random.choice(n_chains, n_chains, p=weights)

        # Create the new distribution of samples to start with
        initial_samples = np.array(
            [initial_samples[new_index] for new_index in new_indices]
            )
        current_sample_obs = np.array(
            [all_current_obs[new_index] for new_index in new_indices]
            )
        current_sample_lls = np.array(
            [all_current_lls[new_index] for new_index in new_indices]
            )
    ...
    # This assumes save_filename ends in ".npy"
    chain_filenames = [
        save_filename[:-4] + f"_chain{i + 1}.npy" for i in range(n_chains)
        ]
