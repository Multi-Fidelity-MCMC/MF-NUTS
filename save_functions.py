...
def add_data(
    ...
    (
        old_all_model_samples,
        old_all_model_obs,
        old_all_model_log_probs,
        old_overall_time,
        old_all_model_times,
        old_overall_acc_ratio,
        old_model_eval_counter,
        old_model_acceptances,
        old_acceptance_history,
        _,
        _,
        _,
        _,
        _,
        old_average_tree_depth,
    ) = load_data(filename)

    if len(all_model_samples.shape) <= 3:  # Adding 1 chain data
        new_all_model_samples = np.vstack(
            (old_all_model_samples, all_model_samples[1:])
            )
        new_all_model_obs = np.vstack((old_all_model_obs, all_model_obs[1:]))
        new_all_model_log_probs = np.vstack(
            (old_all_model_log_probs, all_model_log_probs[1:])
            )
        new_overall_time = old_overall_time + overall_time
        new_all_model_times = old_all_model_times + all_model_times
        new_overall_acc_ratio = (
            overall_acc_ratio * acceptance_history.shape[0]
            + old_overall_acc_ratio * old_acceptance_history.shape[0]
            ) / (new_all_model_samples.shape[0] - 1)
        ...
        new_model_acceptances = np.array([
                (
                    model_acceptances[k] * model_eval_counter[k]
                    + old_model_acceptances[k] * old_model_eval_counter[k]
                )
                / new_model_eval_counter[k]
                if new_model_eval_counter[k] != 0 else 0
                for k in range(len(model_labels))
            ])
        new_acceptance_history = np.hstack(
            (old_acceptance_history, acceptance_history)
            )

        new_average_tree_depth = average_tree_depth
        if average_tree_depth:  # average_tree_depth is not None
            new_average_tree_depth = (
                average_tree_depth * acceptance_history.shape[0]
                + old_average_tree_depth * old_acceptance_history.shape[0]
            ) / (new_all_model_samples.shape[0] - 1)

    else:  # Adding data from multiple chains
        num_chains = acceptance_history.shape[0]

        new_all_model_samples = np.array([
                np.vstack((old_all_model_samples[i], all_model_samples[i][1:]))
                for i in range(num_chains)
            ])
        new_all_model_obs = np.array([
                np.vstack((old_all_model_obs[i], all_model_obs[i][1:]))
                for i in range(num_chains)
            ])
        new_all_model_log_probs = np.array([
                np.vstack((old_all_model_log_probs[i], all_model_log_probs[i][1:]))
                for i in range(num_chains)
            ])
        new_overall_time = np.array([
                old_overall_time[i] + overall_time[i] 
                for i in range(num_chains)
            ])
        new_all_model_times = np.array([
                old_all_model_times[i] + all_model_times[i] 
                for i in range(num_chains)
            ])
        new_overall_acc_ratio = np.array([
                (
                    overall_acc_ratio[i] * acceptance_history[i].shape[0]
                    + old_overall_acc_ratio[i] * old_acceptance_history[i].shape[0]
                )
                / (new_all_model_samples[i].shape[0] - 1)
                for i in range(num_chains)
            ])
        new_model_eval_counter = np.array([
                old_model_eval_counter[i] + model_eval_counter[i]
                for i in range(num_chains)
            ])
        # Be careful in case a fidelity forward model never accepted (count = 0)
        new_model_acceptances = np.array([
                [
                    (
                        model_acceptances[i][k] * model_eval_counter[i][k]
                        + old_model_acceptances[i][k] * old_model_eval_counter[i][k]
                    )
                    / new_model_eval_counter[i][k]
                    if new_model_eval_counter[i][k] != 0 else 0
                    for k in range(len(model_labels))
                ]
                for i in range(num_chains)
            ])
        new_acceptance_history = np.array([
                np.hstack((old_acceptance_history[i], acceptance_history[i]))
                for i in range(num_chains)
            ])

        new_average_tree_depth = average_tree_depth
        if (
            average_tree_depth.all()
        ):  # All elements of average_tree_depth are not None (if one is, they all are)
            new_average_tree_depth = np.array([
                    (
                        average_tree_depth[i] * acceptance_history[i].shape[0]
                        + old_average_tree_depth[i] * old_acceptance_history[i].shape[0]
                    )
                    / (new_all_model_samples[i].shape[0] - 1)
                    for i in range(num_chains)
                ])
    ...
    save_data(
        filename,
        new_all_model_samples,
        new_all_model_obs,
        new_all_model_log_probs,
        new_overall_time,
        new_all_model_times,
        new_overall_acc_ratio,
        new_model_eval_counter,
        new_model_acceptances,
        new_acceptance_history,
        model_labels,
        current_sample_obs,
        current_sample_lls,
        adaptive_matrix,
