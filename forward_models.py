...
def wave_solver(params, x_min, x_max, y_min, y_max, grid_size, dt, timesteps):
    ...
    mv_gauss = lambda coord, mean: 1/(2*jnp.pi) * jnp.exp(-0.5 *
                (jnp.array(coord) - jnp.array(mean)).T @ (jnp.array(coord) - jnp.array(mean)))

    f = lambda x, y: coeff_1 * mv_gauss([x, y], [mean_11, mean_12]) \
    ...
    (x, h), y = jnp.linspace(x_min, x_max, grid_size+1, retstep=True), jnp.linspace(y_min, y_max, grid_size+1)
    X, Y = jnp.meshgrid(x, y)

    # Build the matrix A
    tri_diag = sp.diags(([1]*(grid_size-2), [-4]*(grid_size-1), [1]*(grid_size-2)), offsets=(-1,0,1))
    A = sp.block_diag([tri_diag]*(grid_size-1)) \
        + sp.diags(([1]*((grid_size-1)**2-grid_size+1), [1]*((grid_size-1)**2-grid_size+1)), offsets=(-grid_size+1, grid_size-1))
    A = A.tocsr()
    ...
    temp_layer = U[1:-1, 1:-1] + (theta / 2) * (U[:-2, 1:-1] + U[2:, 1:-1] + U[1:-1, :-2] + U[1:-1, 2:] - 4 * U[1:-1, 1:-1])

    wave_sols = wave_sols.at[1, 1:-1, 1:-1].set(temp_layer)

    wave_sols = propagate(A, wave_sols, grid_size, timesteps)  # Call function to propagate through for-loop
...
def propagate(A, wave_sols, grid_size, timesteps):
    ...
    def _propagate(k: int, args):
        ...
        wave_sols = wave_sols.at[k,1:-1,1:-1].set( (A @ wave_sols[k-1][1:-1,1:-1].flatten()
                                    - wave_sols[k-2][1:-1,1:-1].flatten()).reshape((grid_size-1, grid_size-1)) )
        return (A, wave_sols)
