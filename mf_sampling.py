...
class MF_Sampling:
    """ A class for multi-fidelity MCMC sampling. Methods are provided for multi-fidelity RW and multi-fidelity NUTS.
        Running multi-fidelity RW or NUTS with only one forward model is equivalent to standard RW and standard NUTS,
        respectively. 
        ...
        The following methods must be overwritten for this class to work:
            ...
            add_data()
            save_data()
        In addition, the attribute model_labels defaults to [0, ..., num_models - 1] and may need to be 
        overwritten to better reflect which forward models are being used.
    ...
    def __init__(self, current_sample, current_sample_obs, current_sample_lls, sample_mean, num_models, 
                 adaptive_matrix=None, epsilon=None):
        ...
        current_sample_obs (ndarray):
            The observations mapped from current_sample by each forward model; if they are not provided (i.e. is None), 
            they will be computed automatically
        current_sample_lls (ndarray):
            The log likelihoods of current_sample computed with each forward model; if they are not provided (i.e. is None), 
            they will be computed automatically
        sample_mean (ndarray):
            The sample mean of all drawn samples from the Markov chain, used for matrix adaptation
        num_models (int):
            Indicates how many forward models will be used for multi-fidelity sampling (must be at least 1)
        adaptive_matrix (ndarray):
            The adaptive matrix, corresponding to the step covariance in RW and mass matrix in NUTS
        epsilon (float):
        ...
        # Compute the observations and likelihood of the initial sample if not already provided
        if current_sample_obs is None:
            self.current_sample, self.current_sample_obs, self.current_sample_lls = self.initialize_sample_obs_lls(current_sample, num_models)
        else:
            self.current_sample_obs = current_sample_obs 
            self.current_sample_lls = current_sample_lls 

        self.sample_mean        = sample_mean
        self.num_models         = num_models   # The number of forward models used in multi-fidelity sampling
        ...
        self.model_labels       = np.arange(num_models)   # This array helps label which model is which (should be overwritten)
        self.current_log_prior  = self.log_prior(self.current_sample)

        ### Global statistic trackers ###

        # These three statistics record the samples and corresponding obs/lls accepted by each model
        # If a model rejects a sample, it and all following models repeat the previous sample/obs/ll
        # As such, the final sample/obs/ll of each datapoint are the ones that constitute the posterior
        self.all_model_samples    = [[self.current_sample for _ in range(self.num_models)]]
        self.all_model_obs        = [self.current_sample_obs]
        self.all_model_log_probs  = [[current_sample_ll + self.current_log_prior for current_sample_ll in self.current_sample_lls]]

        self.overall_time           = 0.  # The total time spent running the entire algorithm
        self.overall_acc_ratio      = 0.  # The overall acceptance ratio over all models

        self.all_model_times      = np.zeros(self.num_models, dtype=float)
        self.model_eval_counter   = np.zeros(self.num_models)   # The number of times each model is evaluated
        self.model_acceptances    = np.zeros(self.num_models)   # Number of acceptances for each model
        ...
        self.average_tree_depth   = None  # The average value of binary tree depths used by NUTS
    ...
    def initialize_sample_obs_lls(self, initial_sample, num_models):
        """ Initialize the current state with its observations and likelihoods generated by each forward model.
            This should also perform any parameter transformations to map the initial state to sample space. 
        """
        pass
    ...
    def log_likelihood(self, sample, model):
        """ Computes the log likelihood and forward model observations of the given sample.
            ...
            observations (ndarray):
                The observations returned by the forward model used to compute the likelihood
        """
        pass 
    ...
    def initialize_covariance(self, matrix_adapt_start):
        ...
        # If adaptive_matrix is None, then adaptive_matrix, sample_mean, and sample_cov all need to be initialized (if specified)
        if self.adaptive_matrix is None:
            self.adaptive_matrix = np.eye(self.sample_dim)       # Initialize as the identity
            ...
            # If adaptive_matrix and sample_mean are already specified (i.e. we're continuing a chain),
            # continue adapting adaptive_matrix from the first iteration (if specified)
            if matrix_adapt_start != None:
    ...
    def update_covariance(self, iteration):
        ...
        new_sample_mean = ((iteration + 1) * self.sample_mean + self.current_sample) / (iteration + 2)
        self.sample_cov = (iteration * self.sample_cov + np.outer(self.current_sample, self.current_sample) 
                                            + (iteration + 1) * np.outer(self.sample_mean, self.sample_mean) 
                                            - (iteration + 2) * np.outer(new_sample_mean, new_sample_mean) 
                                            + 0.0001 * np.eye(self.sample_dim)) / (iteration + 1)
    ...
    def initialize_variance(self, matrix_adapt_start):
        ...
        # If epsilon is None, then adaptive_matrix, sample_mean, and sample_var all need to be initialized (if specified)
        if self.epsilon is None:
            self.adaptive_matrix = np.ones(self.sample_dim, dtype=float)  # Initialize mass matrix as the identity
            if matrix_adapt_start != None:
                self.matrix_adapt_start = matrix_adapt_start
                self.sample_mean = self.current_sample.copy()
                self.sample_var = np.zeros_like(self.adaptive_matrix, dtype=float)
        else:
            # If epsilon, adaptive_matrix, and sample_mean are already specified (i.e. we're continuing a chain),
            # continue adapting adaptive_matrix from the first iteration (if specified)
            if matrix_adapt_start != None:
                self.matrix_adapt_start = 0  
                self.sample_var = 1 / self.adaptive_matrix  # Note that M^{-1} = var(samples)
    ...
    def update_variance(self, iteration):
        ...
        new_sample_mean = ((iteration + 1) * self.sample_mean + self.current_sample) / (iteration + 2)
        self.sample_var = (iteration * self.sample_var + self.current_sample**2 
                                            + (iteration + 1) * self.sample_mean**2 
                                            - (iteration + 2) * new_sample_mean**2 
                                            + 0.0001 * np.ones(self.sample_dim, dtype=float)) / (iteration + 1)
        self.sample_mean = new_sample_mean
    ...
    def adapt_epsilon(self, iteration, delta, alpha, n_alpha):
        ...
        self.H_k = (1 - 1 / ((iteration + 1) + self.t0)) * self.H_k + (1 / ((iteration + 1) + self.t0)) * (delta - alpha / n_alpha)
        self.epsilon = np.exp(self.mu - (np.sqrt(iteration + 1) / self.gamma) * self.H_k)
        self.averaged_epsilon = np.exp((iteration + 1)**(-self.kappa) * np.log(self.epsilon) + (1 - (iteration + 1)**(-self.kappa)) * np.log(self.averaged_epsilon))


    def rw_acceptance_probability(self, current_sample_ll, new_sample_ll):
        return new_sample_ll + self.new_log_prior - current_sample_ll - self.current_log_prior
    

    def compute_current_hmc_log_posterior(self, current_p, current_sample_ll):
        self.current_hmc_log_posterior = current_sample_ll + self.current_log_prior - np.sum(current_p**2 / self.adaptive_matrix) / 2


    def compute_new_hmc_log_posterior(self, new_p, new_sample_ll):
        self.new_hmc_log_posterior = new_sample_ll + self.new_log_prior - np.sum(new_p**2 / self.adaptive_matrix) / 2


    def derivative_log_density(self, sample, model):
        """ Computes the derivative of the log density evaluated at the given sample.
            Even if the derivative does not require the likelihood to be evaluated, it 
            should still be evaluated and returned by this function since NUTS requires 
            it for every sample integrated over by the Leapfrog method.
    ...
    def find_reasonable_epsilon(self, current_q, current_grad):
        ...
        self.epsilon = 1
        new_q, new_p, new_ll, _, _ = self.single_leapfrog(current_q, current_p, self.epsilon, current_grad)
    ...
    def build_tree(self, q, p, u, dir, depth, gradient):
        """ Recursively builds the binary tree as outlined in the paper. Note, in this code we return extra values
            for added efficiency: grad_minus, grad_plus, grad_prime, p_prime, hmc_log_prob, ll_prime, and ob_prime.
        """
        
        if depth == 0:
            # Base case -- take one leapfrog step in direction dir
            q_prime, p_prime, ll_prime, ob_prime, grad_prime = self.single_leapfrog(q, p, dir * self.epsilon, gradient)

            with np.errstate(divide='ignore'):  # If u = 0, np.log(u) will throw a divide by zero error unless you use np.errstate()
                logu = np.log(u)

            self.compute_new_log_prior(q_prime)   # Saves as self.new_log_prior
            self.compute_new_hmc_log_posterior(p_prime, ll_prime)  # Saves as self.new_hmc_log_posterior
            ...
            return q_prime, p_prime, grad_prime, q_prime, p_prime, grad_prime, q_prime, p_prime, grad_prime, hmc_log_prob, ll_prime, ob_prime, n_prime, s_prime, np.exp(min(0., hmc_log_prob)), 1
        
        else:
            # Recursion -- implicitly build the left and right subtrees
            q_minus, p_minus, grad_minus, q_plus, p_plus, grad_plus, q_prime, p_prime, grad_prime, hmc_log_prob_prime, ll_prime, ob_prime, \
                n_prime, s_prime, alpha_prime, n_alpha_prime = self.build_tree(q, p, u, dir, depth - 1, gradient)
            if s_prime == 1:
                if dir == -1: 
                    q_minus, p_minus, grad_minus, _, _, _, q_prime_prime, p_prime_prime, grad_prime_prime, hmc_log_prob_prime_prime, ll_prime_prime, ob_prime_prime, \
                        n_prime_prime, s_prime_prime, alpha_prime_prime, n_alpha_prime_prime = self.build_tree(q_minus, p_minus, u, dir, depth - 1, grad_minus)
                else:
                    _, _, _, q_plus, p_plus, grad_plus, q_prime_prime, p_prime_prime, grad_prime_prime, hmc_log_prob_prime_prime, ll_prime_prime, ob_prime_prime, \
                        n_prime_prime, s_prime_prime, alpha_prime_prime, n_alpha_prime_prime = self.build_tree(q_plus, p_plus, u, dir, depth - 1, grad_plus)
                
                # This max() hack ensures that we don't get a divide by 0 error without disrupting the math
                if np.random.uniform() <= (n_prime_prime / max((n_prime + n_prime_prime), 1)):   # Accepted
                ...
                # Note that since the mass matrix is diagonal, M^{-1} @ p = p / mass_matrix
                s_prime = s_prime_prime * ((q_plus - q_minus) @ (p_minus / self.adaptive_matrix) >= 0) * ((q_plus - q_minus) @ (p_plus / self.adaptive_matrix) >= 0)
                n_prime += n_prime_prime 

            return q_minus, p_minus, grad_minus, q_plus, p_plus, grad_plus, q_prime, p_prime, grad_prime, hmc_log_prob_prime, ll_prime, ob_prime, n_prime, s_prime, alpha_prime, n_alpha_prime
    ...
    def update_all_trackers(self, overall_accepted):
        ...
        if overall_accepted == False: # If the high fidelity model did not accept
            ...
            self.temp_model_samples += [self.current_sample for _ in range(model_num, self.num_models)]
            self.temp_model_obs += list(self.current_sample_obs)[model_num:]
            self.temp_model_log_probs += [current_sample_ll + self.current_log_prior 
    ...
    def save_data_reset_trackers(self, save_filename, total_start_time):
        """ Saves data every specified number of iterations and after all sampling """

        # Make sure individual acceptance ratios don't return Nans
        self.model_acceptances = np.array([self.model_acceptances[k] / self.model_eval_counter[k] 
                            if self.model_eval_counter[k] != 0 else 0 
                            for k in range(self.num_models)])
        self.overall_time = time.perf_counter() - total_start_time
        self.overall_acc_ratio = self.current_save_accepted / self.current_save_sample_tracker
    ...
    def sample_multi_fidelity_rw(self, save_filename, n_samples, matrix_adapt_start, save_iters=1000):
        """ Implementation of adaptive Multi-Fidelity RW, saving all data to save_filename
            ...
            matrix_adapt_start (int): 
                The number of iterations at which to start adapting the step covariance matrix
        ...
        for i in range(n_samples):
            ...
            new_sample = multivariate_normal.rvs(mean=self.current_sample, cov=phi*self.adaptive_matrix)

            # Compute the log priors of current and new sample -- ONLY NEED TO COMPUTE ONCE THIS ITERATION
            self.compute_current_log_prior(self.current_sample)
            self.compute_new_log_prior(new_sample)

            alpha_sam_nsam, alpha_nsam_sam = 1., 1.  # Initialize acceptance probabilities for lowest fidelity
            overall_accepted = False  # Indicates whether the sample was accepted by all models
            ...
            for model in range(self.num_models):
                ...
                acceptance = self.rw_acceptance_probability(current_sample_ll, new_sample_ll) \
                                + np.log(alpha_nsam_sam) - np.log(alpha_sam_nsam)

                # Check if sample should be accepted (using log scale)
                alpha_sam_nsam = np.exp(min(0., acceptance)) # alpha(sample, new_sample)
                alpha_nsam_sam = np.exp(min(0., -acceptance)) # alpha(new_sample, sample) but we're dealing with logs still
            ...
            # Update loading bar
            loop.set_description("Acceptance Ratio: {:.4f}".format(self.acceptance_history[-1]))
            loop.update(1)
    ...
    def sample_multi_fidelity_nuts(self, save_filename, n_samples, matrix_adapt_start, epsilon_adapt, 
                                   delta=0.65, save_iters=500):
        """ Implementation of adaptive Multi-Fidelity NUTS, saving all data to save_filename
        ...
        if self.epsilon is None:
            ...
            self.averaged_epsilon = self.epsilon  # epsilon was set as averaged_epsilon in previous run
        ...
        for i in range(n_samples):
            ...
            # Compute the log prior of current sample -- ONLY NEED TO COMPUTE ONCE THIS ITERATION
            self.compute_current_log_prior(self.current_sample)
            ...
            for _ in range(self.max_tree_depth):
                ...
                if direction == -1:
                    q_minus, p_minus, grad_minus, _, _, _, q_prime, p_prime, grad_prime, hmc_log_prob_prime, ll_prime, ob_prime, \
                        n_prime, s_prime, alpha, n_alpha = self.build_tree(q_minus, p_minus, u, direction, depth, grad_minus)
                else:
                    _, _, _, q_plus, p_plus, grad_plus, q_prime, p_prime, grad_prime, hmc_log_prob_prime, ll_prime, ob_prime, \
                        n_prime, s_prime, alpha, n_alpha = self.build_tree(q_plus, p_plus, u, direction, depth, grad_plus)
                ...
                if s_prime == 1 and np.random.uniform() < min(1., n_prime / n):
                    ...
                    # Check if sample should be accepted (using log scale)
                    alpha_sam_nsam = np.exp(min(0., new_hmc_log_prob)) # alpha(sample, new_sample)
                    alpha_nsam_sam = np.exp(min(0., -new_hmc_log_prob)) # alpha(new_sample, sample), but we're dealing with logs still

                n += n_prime
                # Note that since the mass matrix is diagonal, M^{-1} @ p = p / mass_matrix
                s = s_prime * ((q_plus - q_minus) @ (p_minus / self.adaptive_matrix) >= 0) * ((q_plus - q_minus) @ (p_plus / self.adaptive_matrix) >= 0)
                depth += 1
            ...
            if not np.array_equal(new_sample, self.current_sample):
                ...
                # Compute log prior of new sample -- ONLY NEED TO COMPUTE ONCE FOR THE REMAINING ITERATION
                self.compute_new_log_prior(new_sample)
                ...
                for model in range(1, self.num_models):
                    ...
                    # If the new likelihood is (negative) infinity, or if either acceptance ratio is (basically) zero, break out now
                    if np.isinf(new_sample_ll) or np.isclose(alpha_nsam_sam, 0., atol=1e-100) or np.isclose(alpha_sam_nsam, 0., atol=1e-100):
                        self.all_model_times[model] += time.perf_counter() - model_start_time
                        break

                    # Calculate HMC acceptance probability
                    self.compute_new_hmc_log_posterior(new_p, new_sample_ll)
                    new_hmc_log_prob = np.log(alpha_nsam_sam) + self.new_hmc_log_posterior - np.log(alpha_sam_nsam) - self.current_hmc_log_posterior
                ...
                if overall_accepted or self.num_models == 1: # The sample was accepted by all models
                    overall_accepted        = True  # In the case that there's only one model being used
                    self.current_sample     = new_sample
                    self.current_sample_obs = self.temp_new_sample_obs
                    self.current_sample_lls = self.temp_new_sample_lls
            ...
            # Update loading bar
            loop.set_description("Acc Ratio: {:.4f}, Tree Depth: {}, Eps: {:.8f}".format(self.acceptance_history[-1], self.tree_depths[-1], self.epsilon))
            loop.update(1)
