...
if __name__ == "__main__":
    ...
    TRUE_PARAMETERS = np.array([MEAN_11, MEAN_12, MEAN_21, MEAN_22, 
                                COEFF_1, COEFF_2, C])
    ...
    U_sols = forward_models.wave_solver(TRUE_PARAMETERS, XMIN, XMAX, YMIN, YMAX, GRIDSIZE, dt, TIMESTEPS)

    # Scattered Locations [x-location, y-location] where each location is the percentage of 
    # the distance from the min to max value
    ...
    BUOYS = np.array([forward_models.get_buoy(buoy_rat[0], buoy_rat[1], GRIDSIZE)
    ...
    LL_COV_DIAG = np.hstack((
        np.ones_like(OBS_VALS) * 0.03,
        np.ones_like(OBS_TIMES_SECS) * 4
        ))
